...
int alloc_backend_alloc(alloc_device_t* dev, size_t size, int usage, buffer_handle_t* pHandle)
{
    ...
    property_get("sys.cts_gts.status",pro_value,0);
    IsInCts = !strcmp(pro_value,"true") &&!(usage & GRALLOC_USAGE_EXTERNAL_DISP);
    //ALOGD("alloc_backend_alloc pro_value=%s,usage=%x,incts=%d",pro_value,usage,IsInCts);
